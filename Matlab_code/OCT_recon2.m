...
%     folder_distort='/projectnb/npbssmic/ns/PSOCT_grid';                     
...
%     load(strcat(folder_distort,'/curvature_B.mat'));
end

% specify dataset directory
datapath  = strcat('/projectnb/npbssmic/ns/191201_PSOCT/shaded_1000/');
corrected_path=strcat('/projectnb/npbssmic/ns/191201_PSOCT/corrected_1050/');
...
for islice=id
    ...
    for iFile=1:length(filename0)
        ...
        Zsize = 43; Xrpt = 1; Xsize=1000; Yrpt = 1; Ysize = 1000;
...
%              slice = slice(:, 120:Xsize-31, :); %specify FOV cut
%              Slice(21:end,:,:)=slice;
%              slice = FOV_curvature_correction(Slice, curvature_B, size(Slice,1), Xsize-150, Ysize); % specify z and x 
%              slice = Grid_correction(slice, grid_matrix, 1025, 26, 1025, 26, size(slice,1)); % specify x0,x1,y0,y1 and z
%          end
...
%          end
         ...
         aip=squeeze(mean(slice(start_pxl:end,:,:),1));
         mip=squeeze(max(slice(start_pxl:end,:,:),[],1));
